...
def add_dept():
    ...
    if request.method=='POST':
        ...
        mycursor.execute("SELECT count(*) FROM fyp_dept where dept=%s",(dept, ))
        cnt = mycursor.fetchone()[0]
        if cnt==0:
            mycursor.execute("SELECT max(id)+1 FROM fyp_dept")
            maxid = mycursor.fetchone()[0]
            if maxid is None:
                maxid=1
            sql = "INSERT INTO fyp_dept(id, dept,date_join) VALUES (%s, %s, %s)"
            val = (maxid, dept, date_join)
    ...
    return render_template('add_dept.html',msg=msg)
...
def addstu():
    ...
    password=""
    mycursor = mydb.cursor()
    mycursor.execute("SELECT * FROM fyp_dept")
    ...
    return render_template('addstu.html',msg=msg, email=email, mess=mess, st=st, reg_no=reg_no, password=password, name=name, fyp_dept=value)
...
def addstaff():
    ...
    return render_template('addstaff.html',msg=msg, email=email, mess=mess, st=st, staff_id=staff_id, password=password, name=name, fyp_dept=value)
...
@app.route('/view_hod1',methods=['POST','GET'])
def view_hod1():
    ...
    if user_type == 'hod':
        ...
        cursor.execute("SELECT * FROM fyp_staff_allocation WHERE dept = %s", (user_dept,))
        data3 = cursor.fetchall()

    cursor.execute("SELECT DISTINCT reg_no FROM fyp_batch")
    allocated_reg_nos = [row[0] for row in cursor.fetchall()]
    
    # Fetch students who are not allocated to any staff member
    cursor.execute("""
        SELECT s.* 
        FROM fyp_student s
        LEFT JOIN fyp_staff_allocation sa ON s.reg_no = sa.reg_no 
        WHERE sa.reg_no IS NULL
    """)
    ...
    if request.method == 'POST':
        ...
        student_details = {}
        for student_id in selected_students:
            cursor.execute("SELECT reg_no, name FROM fyp_student WHERE id = %s", (student_id,))
            student_info = cursor.fetchone()
            if student_info:
                reg_no = student_info[0]
                name = student_info[1]
                student_details[student_id] = {'reg_no': reg_no, 'name': name}
        ...
        for student_id, details in student_details.items():
            ...
            cursor.execute("SELECT max(id)+1 FROM fyp_batch")
            maxid = cursor.fetchone()[0]
            if maxid is None:
                maxid=1
            cursor.execute("INSERT INTO fyp_batch (id, reg_no, name, tl) VALUES (%s, %s, %s, %s)", (maxid, reg_no, name, selected_staff_id))
...
def abstract():
    ...
    reg_no = session.get('reg_no')
    cursor = mydb.cursor()
    cursor.execute("SELECT * FROM fyp_batch WHERE tl = %s", (reg_no ,))
    d = cursor.fetchall()
    cursor.close()
    ...
    return render_template('abstract.html', msg=msg, msg_type=msg_type, data1=data1, fyp_title=data, sname=sname, fyp_batch=d)
...
def view_report(filename):
    ...
    file_path = os.path.join('E:/fyp/static/uploads', filename)
    ...
    try:
        ...
        pdf_output_path = os.path.join('E:/fyp/static/uploads', f'{filename}.pdf')
